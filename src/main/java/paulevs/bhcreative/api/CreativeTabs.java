...
public class CreativeTabs {
	...
	public static <T extends CreativeTab> T register(T tab) {
		TABS.put(tab.getTranslationKey(), tab);
		TAB_ORDERED.add(tab);
		return tab;
	}
	
	public static void initVanilla() {
		SimpleTab tabFullBlocks = register(new SimpleTab(CreativeMod.id("full_blocks"), (ItemConvertible) BlockBase.STONE));
		SimpleTab tabOtherBlocks = register(new SimpleTab(CreativeMod.id("other_blocks"), (ItemConvertible) BlockBase.LADDER));
		SimpleTab tabTools = register(new SimpleTab(CreativeMod.id("tools"), (ItemConvertible) ItemBase.ironPickaxe));
		SimpleTab tabWeapons = register(new SimpleTab(CreativeMod.id("weapons"), (ItemConvertible) ItemBase.ironSword));
		SimpleTab tabResources = register(new SimpleTab(CreativeMod.id("resources"), (ItemConvertible) ItemBase.ironIngot));
		SimpleTab tabFood = register(new SimpleTab(CreativeMod.id("food"), (ItemConvertible) ItemBase.apple));
		SimpleTab tabItems = register(new SimpleTab(CreativeMod.id("other_items"), (ItemConvertible) ItemBase.slimeball));
		...
		for (int i = 0; i < 2001; i++) {
			if (ItemBase.byId[i] != null) {
				...
				if (i < 256 && BlockBase.BY_ID[i] != null) {
					...
					if (block.isFullCube()) {
						tabFullBlocks.addItem(new ItemInstance(item));
					}
					else {
						...
						tabOtherBlocks.addItem(new ItemInstance(item));
					}
				}
				else if (item instanceof Block) {
					if (item == ItemBase.cake) {
						tabFood.addItem(new ItemInstance(item));
					}
					...
					tabTools.addItem(new ItemInstance(item));
				}
				else if (isWeaponOrArmour(item)) {
					tabWeapons.addItem(new ItemInstance(item));
				}
				else if (isFood(item)) {
					tabFood.addItem(new ItemInstance(item));
				}
				else if (resources.contains(i)) {
					tabResources.addItem(new ItemInstance(item));
				}
				else {
					tabItems.addItem(new ItemInstance(item));
				}
			}
		}
	}
